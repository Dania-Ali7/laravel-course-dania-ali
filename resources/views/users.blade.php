            ...
            @else
                <form action="{{ url('users/create') }}" method="POST">
                    ...
                    <div class="form-group mb-2">
                        <label class="form-label text-muted small font-weight-bold">User Name</label>
                        <input type="text" name="name" class="form-control mb-3" placeholder="Enter user name" required>
                    </div>
        ...
        <div class="card-body p-0">
            <table class="table table-bordered table-striped m-0">
                ...
                <tbody>
                            ...
                            <td class="align-middle">
                                <div class="d-flex">
                                    <form action="{{ url('users/delete/'.$user->id) }}" method="POST" class="mr-2">
                                        @csrf
                                        <button type="submit" class="btn btn-danger btn-sm px-3 d-flex align-items-center font-weight-bold" onclick="return confirm('Are you sure?')">
                                    ...
                                    <form action="{{ url('users/edit/'.$user->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="btn btn-info btn-sm text-white px-3 d-flex align-items-center font-weight-bold">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center p-4 text-muted">No users found. Add a new user above!</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
